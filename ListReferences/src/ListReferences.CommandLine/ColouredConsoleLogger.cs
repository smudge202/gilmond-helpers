...
namespace Gilmond.Helpers.ListReferences.CommandLine
{
	sealed class ColouredConsoleLogger : ILogger
	{
		...
		private OutputStrategy DetermineOutputStrategy(LogLevel logLevel)
		{
			switch (logLevel)
			{
				case LogLevel.Debug:
		...
		sealed class OutputStrategy
		{
			...
			private readonly ConsoleColor _outputColour;

			private OutputStrategy(ConsoleColor outputColour)
			{
			...
			private void Write(object state, Exception exception)
			{
				if (exception == null)
